...
| M4-06 | 选中 Skill 时注入 SKILL.md 正文（第 1 层） | `src/lib/engine/build-context.ts` | 0.5pd |
| M4-07 | Skill 目录路径注入 system prompt，让模型知道去哪读 | 同上 | 0.5pd |
| M4-08 | 上下文 token 统计与预算校验 | 同上 | 0.5pd |
...
| M4-13 | Skill 编辑器（新建/编辑 SKILL.md） | `src/components/skills/skill-editor.tsx` | 0.5pd |
...
- [ ] Demo 能当着人跑通
- [ ] [skill-format.md §11](../specs/skill-format.md) 的 7 个用例全部通过
- [ ] 10 个内置 Skill 全部迁移，产出质量不低于迁移前（逐个对比验证）
- [ ] 未选 Skill 时上下文中的 Skill 索引 < 600 token
- [ ] Web 端降级可用
