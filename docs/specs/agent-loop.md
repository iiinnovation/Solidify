...
2026-09-10 补齐 inline 附件恢复：编译器对与运行时资源精确匹配的完整附件条目施加 512–6000 token 的独立累计预算，保留用户问题和来源，按编号/Markdown 章节生成明确标记为不完整的摘录，并去除重复全文。同一固定交付运行的续写/格式修复保持恢复输入；开放 Agent 获取新工具结果后恢复正常证据预算。`contextStats.inputMode` 和 `inlineRecovery` 记录实际改变，不能只用策略名称推断精简成功。不修改原附件、模型思考参数或输出额度，真实绘图质量仍须验收。原文件复放与限制见 [Draw.io 大附件恢复修复](../drawio-inline-recovery.md)。
