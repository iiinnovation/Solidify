...
把"一次长生成"拆成**五个有界阶段**，模型只在需要判断力的地方出现，网格换算、装配与校验交给确定性代码。
...
用户意图
   ...
   │
   ├─ ② 策划稿     N 次小请求 · 12×6 Bento JSON
   │
   ├─ ③ 逐页生成   N 次小请求 · 参考策划稿和算法区域
   │
   ├─ ④ 装配校验   纯代码 · 无模型
   │
   └─ ⑤ 定向修复   仅失败页回炉 · 带校验错误
...
每页先生成 `PptdPlanningDraft`，只描述卡片类型、内容优先级和 12×6 网格占位，不生成 PPTD 坐标。代码负责约束网格边界、消解卡片重叠，并将网格转换为 960×540 画布中的 `suggestedBounds`。逐页模型输出的元素通过 `planningCardId` 绑定到卡片；代码会把绑定元素确定性地限制在对应卡片区域内，跨卡片连接线可以不绑定。策划稿随检查点持久化，相同输入重试时直接复用。
