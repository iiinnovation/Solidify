...
describe('legacy slide migration', () => {
  it('accepts fenced legacy JSON and rejects empty or unknown layouts', () => {
    expect(parseLegacySlidesDeck('```json\n{"slides":[{"layout":"title","title":"Cover"}]}\n```')?.slides).toHaveLength(1)
    expect(parseLegacySlidesDeck('{"slides":[]}')).toBeNull()
    expect(parseLegacySlidesDeck('{"slides":[{"layout":"unknown"}]}')).toBeNull()
    expect(parseLegacySlidesDeck('not json')).toBeNull()
  })

  it('maps all eight legacy layouts into valid PPTD pages', () => {
    const deck = { slides: layouts.map((layout) => ({
