...
interface CapturePreviewInput { artifact_id?: string; format?: 'png' | 'jpeg' }

export const capturePreviewTool: Tool<CapturePreviewInput> = {
  name: 'capture_preview',
  description: '截取当前 artifact 渲染结果为图片，用于视觉自检。',
  inputSchema: { type: 'object', properties: { artifact_id: { type: 'string' }, format: { type: 'string', enum: ['png', 'jpeg'] } } },
  ...
  async execute(input, _ctx, signal) {
    ...
    try {
      ...
      return success(`已截取预览（${canvas.width}x${canvas.height}）`, { imageDataUrl, width: canvas.width, height: canvas.height, mime })
