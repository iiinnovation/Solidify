      ...
      <main className="min-w-0 flex-1 overflow-y-auto">
        ...
        {selectedTask ? (
          <div className="mx-auto max-w-5xl space-y-5 p-5 lg:p-8">
            ...
            {selectedTask.plan.outputMode === 'review_before_write' && <ResultsCard items={items} />}
...
function ItemStatus({ status }: { status: FolderTaskItem['status'] }) { const labels: Record<FolderTaskItem['status'], string> = { pending: '待处理', processing: '处理中', completed: '已完成', skipped: '已跳过', failed: '失败', pending_decision: '待决策', manual_review: '人工复核', awaiting_external_parser: '等待外部解析器' }; return <span className="shrink-0 text-xs text-text-tertiary">{labels[status]}</span> }
